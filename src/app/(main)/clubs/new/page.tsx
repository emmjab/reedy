  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
        <Input label="Club name" id="name" value={name} onChange={(e) => setName(e.target.value)} required />
        <div>
        ...
        <label className="flex cursor-pointer items-center gap-3">
          ...
          <span className="text-sm text-gray-700">Make this club public (anyone can join)</span>
        </label>
        {error && <p className="text-sm text-red-600">{error}</p>}
        <div className="flex gap-3">
          <Button type="submit" loading={loading} className="flex-1">Create club</Button>
